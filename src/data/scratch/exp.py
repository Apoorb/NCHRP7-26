...
def weibull_fn(flow_seq, breakdown_probability):
    breakdown_probability = np.array(breakdown_probability)
    breakdown_probability[
        np.where(breakdown_probability == 0)
    ] = 0.0000001  # identifying breakdowns with 0% prob.
...
def output_file(
    ...
    filename = "Data_output_without_removal.csv"
    if os.path.isfile(filename):
        with open("Data_output_without_removal.csv", "a") as f:
            f.write(
                database.split("_")[0]
                + ","
...
if __name__ == "__main__":
    ...
    path_to_out_files = r"C:\Users\abibeka\Github\NCHRP 7-26\data\interim\pre_breakdown_data_no_removals"
