...
if __name__ == "__main__":
    ...
    prebreakdown_merge_vol_fil = prebreakdown_merge.loc[
        ...
        & (df.geometry_type.isin(
            ["Close merge", "Simple merge", "Two lane on ramp"]
        ))
    ]
    ...
    prebreakdown_merge_len_acc_1500 = one_hot_coding_cat(prebreakdown_merge_len_acc_1500)

    formula_ = "ramp_metering+length_of_acceleration_lane+ffs_cap_df" \
              "+number_of_mainline_lane_downstream+simple_merge"
    aft_all_data = fit_aft_model(prebreakdown_merge_vol_fil,
                  formula_=formula_)
    aft_all_data.print_summary(decimals=5)
    aft_all_data.median_survival_time_
    formula_ = "1"
    aft_intercept = fit_aft_model(prebreakdown_merge_vol_fil,
                  formula_=formula_)
    ...
    for i, ax in enumerate(fig.axes):
        ...
        aft_plots_on_survival(
            model=aft_all_data,
            var=var,
            var_levels=var_levels,
            ax=ax
        )
        if var == "simple_merge": break
    ...
    sum_df=prebreakdown_merge_vol_fil.drop_duplicates("file_name").geometry_type.value_counts()
    sum_df.to_csv(
    ...
    formula_ = "ramp_metering+length_of_acceleration_lane+ffs_cap_df+fwy_to_fwy_ramp" \
              "+number_of_mainline_lane_downstream+simple_merge"
    aft_all_1500_acc = fit_aft_model(prebreakdown_merge_len_acc_1500,
                  formula_=formula_)
